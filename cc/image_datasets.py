...
def load_lmdb(
    *,
    ...
    def preprocess(pil_image):
        if random_crop:
            arr = random_crop_arr(pil_image, image_size)
        else:
    ...
    dataset = LSUN(
        ...
        target_transform=label_process
    )
    if deterministic:
        loader = DataLoader(
            dataset, batch_size=batch_size, shuffle=False, num_workers=1, drop_last=True
...
class NpyDataset(Dataset):
    def __init__(self, path, shard=0, num_shards=1):
        super().__init__()
        assert os.path.exists(path), ".npy dataset file not found!"
        arr = np.load(path)
        self.local_arr = arr[shard:][::num_shards]
        self.local_arr = self.local_arr.astype(np.float32)

    def __len__(self):
        return len(self.local_arr)

    def __getitem__(self, idx):
        return self.local_arr[idx][None, ...], {}
